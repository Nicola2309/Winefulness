...
```
git clone https://github.com/Nicola2309/Winefulness.git
```
...
```
11. With this complete, create a superuser for your project with the following command and follow the instructions in the Terminal (note: this will be necessary to add data to the wineshop site once deployed locally):
```
...
2. Create an account and sign in to Heroku. From here, created a new app with a unique name that had not already been taken (this project uses 'winefulness'). Set the region to the closest to you, eg. 'Europe'.
...
```
echo web: gunicorn winefulness.wsgi:application > Procfile
```
